    ...
    <section className="mb-8">
      ...
      <div className="grid grid-cols-4 gap-4">
        {COLOR_THEMES.map((t) => (
          ...
          >
            <div
              className={`w-12 h-12 rounded-full ${t.color} flex items-center justify-center transition-all group-hover:scale-110 shadow-lg ${
                colorTheme === t.id
                  ? 'ring-4 ring-white ring-offset-2 ring-offset-gray-900 shadow-blue-500/50'
                  : ''
              }`}
            >
              {colorTheme === t.id && <Check className="w-6 h-6 text-white drop-shadow-md" />}
            </div>
            <span
              className={`text-[10px] uppercase font-bold tracking-tighter ${
                colorTheme === t.id ? 'text-white' : 'text-gray-500'
              }`}
            >
              {t.id}
            </span>
