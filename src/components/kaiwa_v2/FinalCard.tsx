    ...
    <div className="border border-gray-200 rounded-lg p-4 relative bg-white shadow-sm">
      ...
      <div className="space-y-4">
        {conv.dialogue.map((line, idx) => (
          <div
            key={idx}
            className="flex gap-4 items-start group hover:bg-gray-50 p-2 rounded-lg transition-colors"
          >
            ...
            <div className="flex-1">
              ...
              {showEnglish && <div className="text-sm text-gray-600">{line.english}</div>}
            </div>
            {/* Play Button */}
            <button
              onClick={() =>
                playingLine === idx
                  ? stopAudio()
                  : playAudio(stripFurigana(line.japaneseWithFurigana), idx)
              }
